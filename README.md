...
현재 이 저장소에는 내가 전역에서 사용 중인 아래 8개 로컬 스킬이 보조 파일까지 포함된 형태로 들어 있습니다.
...
* 다른 PC에서 이 저장소만 clone 해도 개인 로컬 스킬 8개의 원문을 바로 확보할 수 있게 하기
...
* `write-codebase-language-tutorial`은 `agents/openai.yaml`, `assets/tutorial-template.html`, `references/beginner-pedagogy.md`, `references/language-entrypoints.md`, `scripts/validate_tutorial.py`를 포함합니다.
...
cp -R ~/.codex/personal-skills/skills/write-codebase-language-tutorial ~/.codex/skills/
```
...
5. Copy these full local skill directories into ~/.codex/skills:
   ...
   - ~/.codex/personal-skills/skills/write-codebase-language-tutorial
...
ls -la ~/.codex/skills/write-codebase-language-tutorial
playwright-cli --version
```
...
```text
Use $write-codebase-language-tutorial to inspect this service and write a zero-assumption language tutorial grounded in its actual code.
```
...
skills/
  ...
  write-codebase-language-tutorial/
    ...
    references/
      beginner-pedagogy.md
      language-entrypoints.md
    scripts/
